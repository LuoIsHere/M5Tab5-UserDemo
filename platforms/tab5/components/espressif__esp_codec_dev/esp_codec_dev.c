...
int esp_codec_dev_open(esp_codec_dev_handle_t handle, esp_codec_dev_sample_info_t *fs)
{
    ...
    if (dev->input_opened || dev->output_opened) {
        ESP_LOGI(TAG, "Codec device already open");
        return ESP_CODEC_DEV_OK;
    }
    ...
    if (open_input == false && open_output == false) {
        return ESP_CODEC_DEV_NOT_SUPPORT;
    }

    esp_codec_dev_type_t open_caps = (esp_codec_dev_type_t) 0;
    if (open_input) {
        open_caps = (esp_codec_dev_type_t) (open_caps | ESP_CODEC_DEV_TYPE_IN);
    }
    if (open_output) {
        open_caps = (esp_codec_dev_type_t) (open_caps | ESP_CODEC_DEV_TYPE_OUT);
    }
    ...
    bool sw_vol_opened = false;
    int ret = ESP_CODEC_DEV_OK;

    if (data_if->set_fmt) {
        ret = data_if->set_fmt(data_if, open_caps, fs);
        if (ret != ESP_CODEC_DEV_OK) {
            ESP_LOGE(TAG, "Fail to set data interface format: %d", ret);
            goto rollback;
        }
    }
    if (data_if->enable) {
        ret = data_if->enable(data_if, open_caps, true);
        if (ret != ESP_CODEC_DEV_OK) {
            ESP_LOGE(TAG, "Fail to enable data interface: %d", ret);
            goto rollback;
        }
        data_enabled = true;
    }
    if (codec) {
        if (codec->set_fs) {
            ...
            if (ret != ESP_CODEC_DEV_OK) {
                ESP_LOGE(TAG, "Fail to set codec sample format: %d", ret);
                goto rollback;
            }
        }
        if (codec->enable) {
            ...
            if (ret != ESP_CODEC_DEV_OK) {
                ESP_LOGE(TAG, "Fail to enable codec: %d", ret);
                goto rollback;
            }
            codec_enabled = true;
        }
    ...
    if (open_output) {
        if (codec == NULL || codec->set_vol == NULL) {
            if (dev->sw_vol == NULL) {
                ...
                dev->sw_vol_alloced = (dev->sw_vol != NULL);
            }
        }
        if (dev->sw_vol) {
            ret = dev->sw_vol->open(dev->sw_vol, fs, VOL_TRANSITION_TIME);
            if (ret != ESP_CODEC_DEV_OK) {
                ESP_LOGE(TAG, "Fail to open software volume: %d", ret);
                goto rollback;
            }
            sw_vol_opened = true;
        }
    ...
    dev->input_opened = open_input;
    dev->output_opened = open_output;
...
rollback:
    if (sw_vol_opened && dev->sw_vol && dev->sw_vol->close) {
        dev->sw_vol->close(dev->sw_vol);
    }
    if (codec_enabled && codec && codec->enable) {
        codec->enable(codec, false);
    }
    if (data_enabled && data_if->enable) {
        data_if->enable(data_if, open_caps, false);
    }
