...
#define I2S_WRITE_WAIT_TIMEOUT_MS      (100)
#define I2S_WRITE_TOTAL_TIMEOUT_MS     (1000)
...
{
    i2s_data_t *i2s_data = (i2s_data_t *) h;
    if (i2s_data == NULL || data == NULL || size <= 0) {
        return ESP_CODEC_DEV_INVALID_ARG;
    }
    if (i2s_data->is_open == false) {
        return ESP_CODEC_DEV_WRONG_STATE;
    ...
    if (i2s_data->out_reconfig) {
        ESP_LOGW(TAG, "Reject write while output is being reconfigured");
        return ESP_CODEC_DEV_WRONG_STATE;
    }
    ...
    while (total_written < (size_t) size) {
        ...
        int ret = i2s_channel_write(tx_chan, data + total_written, (size_t) size - total_written,
                                    &bytes_written, I2S_WRITE_WAIT_TIMEOUT_MS);
        if (bytes_written > ((size_t) size - total_written)) {
            ESP_LOGE(TAG, "Invalid I2S write count: %u", (unsigned) bytes_written);
            return ESP_CODEC_DEV_WRITE_FAIL;
        }
        total_written += bytes_written;

        if (total_written == (size_t) size) {
            return ESP_CODEC_DEV_OK;
        }
        if (ret != ESP_OK && ret != ESP_ERR_TIMEOUT) {
            ESP_LOGE(TAG, "I2S write failed: ret=%d written=%u/%u", ret,
                     (unsigned) total_written, (unsigned) size);
            return ESP_CODEC_DEV_DRV_ERR;
        }
        if (bytes_written == 0) {
            ESP_LOGE(TAG, "I2S write made no progress: ret=%d written=%u/%u", ret,
                     (unsigned) total_written, (unsigned) size);
            return ESP_CODEC_DEV_WRITE_FAIL;
        }
        if ((xTaskGetTickCount() - start_tick) >= pdMS_TO_TICKS(I2S_WRITE_TOTAL_TIMEOUT_MS)) {
            ESP_LOGE(TAG, "I2S full-write timeout: written=%u/%u",
                     (unsigned) total_written, (unsigned) size);
            return ESP_CODEC_DEV_WRITE_FAIL;
        }
    }
    ...
    return (ret == 0 && bytes_written == (size_t) size) ? ESP_CODEC_DEV_OK : ESP_CODEC_DEV_WRITE_FAIL;
#endif
}
